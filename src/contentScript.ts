...
function parsePreferencesTable(preferenceGrid: Element) {
...
async function updatePreferences(
  originalGrid: HTMLElement,
  currentGrid: HTMLElement,
) {
  const originalPreferences: { [key: string]: boolean } = {};
  const currentPreferences: { [key: string]: boolean } = {};

  // Parse original preferences
  const originalCheckboxes = originalGrid.querySelectorAll(
    'input[type="checkbox"]',
  ...
  const currentCheckboxes = currentGrid.querySelectorAll(
    'input[type="checkbox"]',
  ) as NodeListOf<HTMLInputElement>;
  ...
  const checkboxesToUpdate: string[] = [];
  ...
  for (const id of checkboxesToUpdate) {
    const label = currentGrid.querySelector(
      `label[for="${id}"]`,
    ) as HTMLLabelElement;
    if (label) {
      label.click();
      ...
      await new Promise<void>((resolve) => {
        const checkForChange = () => {
          const checkbox = currentGrid.querySelector(
            `#${id}`,
          ) as HTMLInputElement;
