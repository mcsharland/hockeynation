...
class CoachMarketVisualizer {
	...
	private onTableUpdated(): void {
		console.log(
			"[coach-market] onTableUpdated, container connected:",
	...
	private injectModalCheckboxes(modal: HTMLElement): void {
		...
		const minMaxInput = modal.querySelector<HTMLInputElement>("#hn-min-max");
	...
	private renderColumnGroup(group: keyof ColumnGroupState) {
		...
		const parentHeader = Array.from(this.header.querySelectorAll("th")).find(
			(th) => th.textContent?.trim() === parentHeaderText,
		);
	...
	private syncColumnStateFromTable(): void {
		...
		const hasOvr =
			this.header.querySelector('th:not([data-column^="hn-"])') !== null &&
